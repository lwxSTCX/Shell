...
if [ ! -n "$checks"  ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
		...
		rpm -e sssd-client
		if [ $? -eq 0 ];then
			echo "删除组件：sssd-client" | tee -a out.txt
		fi
		rpm -e poppler --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：poppler" | tee -a out.txt
		fi
		rpm -e poppler-utils --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：poppler-utils" | tee -a out.txt
		fi
		rpm -e pango --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：pango" | tee -a out.txt
		fi
		rpm -e libcroco --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：libcroco" | tee -a out.txt
		fi
		rpm -e harfbuzz --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：harfbuzz" | tee -a out.txt
		fi
		rpm -e gobject-introspection --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：gobject-introspection" | tee -a out.txt
		fi
		rpm -e freetype --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：freetype" | tee -a out.txt
		fi
		rpm -e fontconfig --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：fontconfig" | tee -a out.txt
		fi
		rpm -e cairo --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：cairo" | tee -a out.txt
		fi
		rpm -e ghostscript --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：ghostscript" | tee -a out.txt
		fi
		rpm -e ghostscript-cups --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：ghostscript-cups" | tee -a out.txt
		fi
		rpm -e libjpeg-turbo --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：libjpeg-turbo" | tee -a out.txt
		fi
		rpm -e libwbclient --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：libwbclient" | tee -a out.txt
		fi
		rpm -e graphite2 --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：graphite2" | tee -a out.txt
		fi
		rpm -e nss-sysinit --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：nss-sysinit" | tee -a out.txt
		fi
		rpm -e nss-tools --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：nss-tools" | tee -a out.txt
		fi
		rpm -e net-snmp-libs --nodeps
		if [ $? -eq 0 ];then
			echo "删除组件：net-snmp-libs" | tee -a out.txt
		fi
		rpm -e poppler-data
		if [ $? -eq 0 ];then
			echo "删除组件：poppler-data" | tee -a out.txt
		fi
		rpm -e ghostscript-fonts
		if [ $? -eq 0 ];then
			echo "删除组件：ghostscript-fonts" | tee -a out.txt
		fi
    fi
