...
if [ -n "$no_repeat_password_1" -a -e "$no_repeat_password_file" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        no_repeat_password_3=`cat /etc/pam.d/system-auth |grep 'password    required        pam_deny.so'`
        if [ -n "$no_repeat_password_3" ];then
            sed -i '/password    required        pam_deny.so/a\ remember=5' /etc/pam.d/system-auth
        else
            echo 'password    required        pam_deny.so remember=5' >> /etc/pam.d/system-auth
        fi
        if [ $? -eq 0 ];then
...
if [ ! -e "$temp_1" ];then
	...
	file_power_6=`ls -l /etc/xinetd.conf | awk '{print $1}'`
fi
if [ ! -e "$temp_2" ];then
    file_power_7="-rw-------"
else
	file_power_7=`ls -l /etc/grub.conf | awk '{print $1}'`
fi
if [ ! -e "$temp_3" ];then
    file_power_8="-rw-------"
else
	file_power_8=`ls -l /etc/lilo.conf | awk '{print $1}'`
fi
file_power_1=`ls -l /etc/passwd | awk '{print $1}'`
file_power_2=`ls -l /etc/shadow | awk '{print $1}'`
file_power_3=`ls -l /etc/group | awk '{print $1}'`
...
ssh_open=`netstat -an | grep '0.0.0.0:22' | grep 'LISTEN'`
telnet_close=`netstat -an | grep '0.0.0.0:23'`
...
#升级SSH版本（ok）
#centos 7.1/7.4 特定版本更新
system_limit=`uname -r |cut -c 8-10`
ssh_version=`rpm -qa |grep openssh-[0-9].[0-9] |cut -c 9-11`
if [ $system_limit == 229 ] || [ $system_limit == 693 ] || [ $system_limit == 957 ];then 
    if [ $ssh_version  = 8.1 ];then
        echo "ssh版本满足要求，不用执行升级" >> out.txt
    else
        echo 'ssh版本不满足要求，是否立即升级：(Y/N):'
        read user_num_YorN_re
        temp=`echo ${user_num_YorN_re} | grep Y`
        temp1=`echo ${user_num_YorN_re} | grep y`
        if [ "$temp" != "" -o "$temp1" != "" ];then
            #安装ssl
            if [ -e "HW_install.tar.gz" ];then
                tar xzvf HW_install.tar.gz
                cd HW_install
                rpm -Uvh openssl* --nodeps --force
                openssl version
            else
                echo "openssh升级安装包不存在" >> out.txt
            fi
            if [ "$?" -eq 0 ];then
                cd /root/HW_install
                ...
                if [ "$?" -eq 0 ];then
                    ...
                    echo "openssh安装成功:" >> out.txt
                fi
            fi          
        fi
    fi
else
    echo "centos版本不满足要求，无法进行ssh升级" >> out.txt
fi
...
if [ -n "$port_server" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        do
            chkconfig $port_server_close off
