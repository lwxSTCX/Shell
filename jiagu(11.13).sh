...
if [ ! -n "$password_no"  ];then
    echo "系统中没有空密码账号，符合要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        echo '已为账号设置密码:'$no_password_user | tee -a out.txt
    fi
fi
...
if [ -n "$temp_no_user_3" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        do
                ...
                echo '已锁定无关账号'$no_use_user | tee -a out.txt
        done
    fi
else
    echo "系统中不存在未锁定账号"
...
if [ -n "$no_repeat_password_1" -a -e "$no_repeat_password_file" ];then
    echo "不重复使用最近的口令，符合要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ $? -eq 0 ];then
            echo '不重复使用最近的口令已完成配置' | tee -a out.txt
        fi
    fi
fi

#按照用户角色分配不同权限的账号
echo "自行检查是否按照用户角色分配不同权限的账号，输出结果至out.txt----------"
echo "请自行根据需求改账户权限" >> out.txt
echo "####################################################" | tee -a out.txt
echo `cat /etc/passwd | awk -F: '{ print $1"---"$6"---"$7 }'` | tee -a out.txt
echo "####################################################" | tee -a out.txt

#禁止超级管理员账户远程登录（ok）
echo "检查是否禁止超级管理员账户远程登录-------------"
...
if [  -e "$no_remote_root_file1" -a  -e "$no_remote_root_file2" -a  -e "$no_remote_root_file3" -a  -e "$no_remote_root_file4" ];then
    ...
    if [  -n "$no_remote_root_1" -a  -n "$no_remote_root_2" -a  -n "$no_remote_root_3" -a  -n "$no_remote_root_4" ];then
        echo "禁止超级管理员账户远程登录，符合要求" | tee -a out.txt
    else
        ...
        if [ "$temp" != "" -o "$temp1" != "" ];then
            ...
            if [ ! -n "$no_remote_root_1" -o ! -n "$no_remote_root_2" ];then
                ...
                echo '禁止root用户远程telnet登录系统;配置完成'  | tee -a out.txt
            fi
            if [ ! -n "$no_remote_root_3"  ];then
                ...
                echo '禁止root用户远程ssh登录系统,配置完成'  | tee -a out.txt
            fi
            if [ "$no_remote_root_4" -lt "2" ];then
                ...
                echo '禁止root图形界面登录，配置完成'  | tee -a out.txt
            fi
            if [ "$?" -eq 0 ];then
                echo '禁止超级管理员账户远程登录已完成配置' | tee -a out.txt
            fi
        fi
    fi
fi
...
if [ -n "$auth_false_1" -a -n "$auth_false_2" ];then
    echo "连续认证失败次数，配置满足要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            echo '连续认证失败次数配置完成' | tee -a out.txt
        fi
    fi
fi

#删除root为UID为0的用户（ok）
echo "删除root为UID为0的用户--------------"
...
if [ "$root_UID_0" = "root" -a "$root_UID_0_line" -lt 2 ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            echo '删除root以外UID为0的账户配置完成' | tee -a out.txt
        fi
    fi
fi
...
if [ "$system_umask_1" = "027" -a "$system_umask_2" = "027" -a "$system_umask_3" = "027" -a "$system_umask_4" = "027" -a "$system_umask_5" = "027" -a "$system_umask_6" = "027" ];then
    echo "系统umask设置配置满足要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            ...
            echo '系统umask设置配置完成' | tee -a out.txt
        fi
    fi
fi
...
if [ ! -n "$sec_romote_user_1"  -a  ! -n "$sec_romote_user_2"  -a  ! -n "$sec_romote_user_3"  ];then
    echo "账号与口令安全性设置满足要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            ...
            rm -f /etc/.rhosts
            rm -f /etc/.equiv
            echo '账号与口令安全性设置配置完成' | tee -a out.txt
        fi
    fi
fi
...
if [ -n "$gconftool_tmp" ];then
    echo "gconftool命令不存在,进行下一项检查"  | tee -a out.txt
else
    ...
    if [ -n "$screen_lock_1"  -a -n "$screen_lock_2" -a -n "$screen_lock_3" -a -n "$screen_lock_4" ];then
        ...
        if [ "$temp" != "" -o "$temp1" != "" ];then
            ...
            if [ "$?" -eq 0 ];then
                echo '屏幕锁定要求设置配置完成' | tee -a out.txt
            fi
        fi
    fi
fi
...
if [  -n "$pam_root_1"  -a  -n "$pam_root_1" ];then
    echo "使用PAM禁止任何人为su为root满足要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            echo '使用PAM禁止任何人为su为root设置配置完成' | tee -a out.txt
        fi
    fi
fi

#检查root目录权限是否为700（ok）
echo "检查root目录权限是否为700-------------------"
...
if [ "$check_root_700" = "-rwx------." -o "$check_root_700" = "drwx------." ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        chmod 0700 /root
        if [ "$?" -eq 0 ];then
            echo 'root目录权限为700设置配置完成' | tee -a out.txt
        fi
    fi
fi
...
if [  "$system_user_login_2" -eq "$system_user_login_1" ];then
    echo "系统账户登录限制满足要求" | tee -a out.txt
else
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
		...
		if [ "$?" -eq 0 ];then
			echo '系统账户登录限制设置配置完成' | tee -a out.txt
		fi
    fi
fi
...
if [ -e "$centos_file" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        if [ "$?" -eq 0 ];then
            echo '补充重要目录和文件的权限设置配置完成' | tee -a out.txt
        fi
    fi
fi
...
if [ ! -e "$syslog_log" ];then
    echo "/etc/syslog.conf不存在，进入下一项"  | tee -a out.txt
else
    ...
    if [  -n "$syslog_log_tmp_1" -a -n "$syslog_log_tmp_2" ];then
        echo "记录登录认证和权限变更事件的配置，符合要求" | tee -a out.txt
    else
        ...
        if [ "$temp" != "" -o "$temp1" != "" ];then
            ...
            if [ "$?" -eq 0 ];then
                ...
                echo '记录登录认证和权限变更事件设置配置完成' | tee -a out.txt
            fi
        fi
    fi
fi
...
if [ ! -e "$system_log_power_1" -a ! -e "$system_log_power_2" ];then
    echo "系统/dev/sysmsg，/var/adm/messages文件均不存在"  | tee -a out.txt
else
...
if [ ! -e "$syslog_log" ];then
    ...
    if [  -n "$syslog_log_tmp_1" ];then
        echo "配置日志记录系统安全事件，符合要求" | tee -a out.txt
    else
...
if [ ! -e "$syslog_log" ];then
    ...
    if [  -n "$syslog_log_tmp_1" -a -n "$syslog_log_tmp_2" ];then
        echo "系统应用/服务log配置，符合要求" | tee -a out.txt
    else
...
if [ ! -e "$syslog_log" ];then
    ...
    if [  -n "$syslog_log_tmp_1" ];then
        echo "设置日志服务器远程功能，符合要求" | tee -a out.txt
    else
...
if [ $? -eq 0 ];then
	...
	if [  -n "$ssh_open" -a ! -n "$telnet_close" ];then
		echo "远程登录取消telnet采用ssh，符合要求" | tee -a out.txt
	else
...
if [ ! -e "$syslog_log" ];then
    ...
    if [ -n "$syslog_log_tmp_1" ];then
        echo "优化ssh的banner信息，符合要求" | tee -a out.txt
    else
...
if [ - n "$telnet_banner" ];then
    ...
    if [ ! -e "$telnet_banner_tmp_1" ];then
        ...
        if [ -n "$telnet_banner_tmp_1_1" ];then
            echo "/etc/issue优化telnet的Banner信息，符合要求" | tee -a out.txt
        else
    ...
    if [ ! -e "$telnet_banner_tmp_2" ];then
        ...
        if [ -n "$telnet_banner_tmp_1_2" ];then
            echo "/etc/issue.net优化telnet的Banner信息，符合要求" | tee -a out.txt
        else
            echo '优化telnet的Banner信息不符合要求，是否立即设置：(Y/N):'
            read user_num_YorN_re
            temp=`echo ${user_num_YorN_re} | grep Y`
...
if [ "$icmp_redirects" -eq 0 ];then
    echo "禁止ICMP重定向配置满足要求"  | tee -a out.txt
else
    icmp_tmp='/etc/sysctl.conf'
    if [ ! -e "$icmp_tmp" ];then
        echo "/etc/sysctl.conf文件不存在" | tee -a out.txt
    else
...
if [ "$route_redirects" -eq 0 ];then
    echo "禁止源路由转发配置满足要求"  | tee -a out.txt
else
...
if [ -e "$remote_ip_allow"  ];then
    ...
    if [  -n "$remote_ip_allow_tmp" ];then
        echo "/etc/hosts.allow设置了允许访问的IP，配置满足要求"  | tee -a out.txt
    else
...
if [ -e "$remote_ip_deny"  ];then
    ...
    if [  -n "$remote_ip_allow_tmp" ];then
        echo "/etc/hosts.deny设置了禁止访问的IP，配置满足要求"  | tee -a out.txt
    else
...
if [ -e "$core_tmp"  ];then
    ...
    if [  "$coredump" -eq 0 ];then
        echo "coredump配置满足要求"  | tee -a out.txt
    else
...
if [ -e "$NTP"  ];then
    ...
    if [  -n "$NTP_tmp" ];then
        echo "/etc/ntp.conf配置NTP服务器，配置满足要求"  | tee -a out.txt
    else
...
if [ -n "$port_server" ];then
    ...
    do
        ...
        echo "系统中未禁用的不必要服务:"$port_server_tmp  | tee -a out.txt
    done
else    
...
if [ -n "$port_server" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        do
            ...
            echo '已关闭不必要服务'$port_server_close | tee -a out.txt
        done
    fi
    if [ "$?" -eq 0 ];then
        echo "已关闭不必要的服务配置完成"
...
if [  -e "$profile" ];then
    ...
    if [ -n "$profile_TMOUT" ];then
        echo "/etc/profile文件中有TMOUT，符合要求" | tee -a out.txt
    else
...
if [  -e "$csh_file" ];then
    ...
    if [ -n "$csh" ];then
        echo "/etc/csh.cshrc文件中有autologout，符合要求" | tee -a out.txt
    else
...
if [ -e "$system_bug" ];then
    ...
    echo "/var/sadm/patch文件不存在"  | tee -a out.txt
fi

#检查系统已安装的不必要软件包及需要更新的软件包（ok）
echo "检查系统已安装的不必要软件包及需要更新的软件包-------------"
echo "#############################################################" >> out.txt
...
if [ -n "$NFS_service" ];then
    ...
    do
        ...
        echo "系统中存在的NFS服务:"$NFS_service_tmp  | tee -a out.txt
    done
else    
...
if [ -n "$NFS_service_1" ];then
    ...
    if [ "$temp" != "" -o "$temp1" != "" ];then
        ...
        do
            ...
            echo '已关闭不必要NFS服务pid:'$NFS_service_tmp_1 | tee -a out.txt
        done
    fi
    if [ "$?" -eq 0 ];then
        echo "NFS服务限制配置完成"
...
# if [  -e "$lilo_file" ];then
    ...
    # if [ -n "$lilo_file_tmp" ];then
        # echo "/etc/lilo.conf文件中有password且未被注释，符合要求" | tee -a out.txt
    # else
...
# if [  -e "$grub_file" ];then
    ...
    # if [ -n "$grub_file_tmp" ];then
        # echo "/etc/grub.conf文件中有password且未被注释，符合要求" | tee -a out.txt
    # else
...
if [  -e "$grub_file_1" ];then
    echo "检查设置FTP的/etc/ftpusers访问权限"  | tee -a out.txt
    echo `more /etc/ftpusers`  | tee -a out.txt
else
    echo "/etc/ftpusers文件不存在"  | tee -a out.txt
fi
grub_file_2='/etc/vsftpd.ftpusers'
if [  -e "$grub_file_2" ];then
    echo "检查设置FTP的/etc/vsftpd.ftpusers访问权限"  | tee -a out.txt
    echo `/etc/vsftpd.ftpusers`  | tee -a out.txt
else
    echo "/etc/vsftpd.ftpusers文件不存在"  | tee -a out.txt
fi
grub_file_3='/etc/ftpaccess'
if [  -e "$grub_file_3" ];then
    echo "检查设置FTP的/etc/ftpusers访问权限"  | tee -a out.txt
    echo `more /etc/ftpaccess`  | tee -a out.txt
else
    echo "/etc/ftpusers文件不存在"  | tee -a out.txt
fi
grub_file_4='/etc/vsftpd/vsftd.conf'
if [  -e "$grub_file_4" ];then
    echo "检查设置FTP的/etc/vsftpd/vsftd.conf访问权限"  | tee -a out.txt
    echo `more /etc/vsftpd/vsftd.conf`  | tee -a out.txt
else
    echo "/etc/vsftpd/vsftd.conf文件不存在"  | tee -a out.txt
fi
...
if [  -e "$vsftpd_file" ];then
    ...
    if [ -n "$vsftpd_file_tmp_1" -a -n "$vsftpd_file_tmp_2" ];then
        echo "/etc/vsftpd/vsftpd.conf配置，符合要求" | tee -a out.txt
    else
...
if [ -e "$vsftpd_file" ];then
    ...
    if [ -n "$vsftpd_file_tmp" ];then
        echo "/etc/vsftpd.conf配置，符合要求" | tee -a out.txt
    else
...
if [ -e "$pure_ftpd_file" ];then
    ...
    if [ -n "$pure_ftpd_file_tmp" ];then
        echo "/etc/pure-ftpd/pure-ftpd.conf'配置，符合要求" | tee -a out.txt
    else
...
do
    if [ $disk_tmp -gt 80 ];then
        disk_tmp_1=`df -h | grep $disk_tmp | awk '{ print $6 }'`
        echo "磁盘不满足要求的为："$disk_tmp_1 ":" $disk_tmp "请尽快清理" | tee -a out.txt
    else
        echo $disk_tmp_1"磁盘空间充足，满足要求" >> out.txt
    fi
done
...
if [ -e "$tcp_6000" ];then
    ...
    echo "/etc/X11/xdm/Xservers文件不存在"  | tee -a out.txt
fi
...
if [ -e "$tcp_6000_1" ];then
    ...
    echo "/etc/X11/xinit/xserverrc文件不存在"  | tee -a out.txt
fi
...
if [ -e "$system_banner" ];then
	...
	issue_temp='/etc/issue'
	if [ -e "$issue_temp" ];then
		mv /etc/issue /etc/issue.bak
		rm /etc/issue
	fi 
	issue.net_temp='/etc/issue.net'
	if [ -e "$issue.net_temp" ];then
...
echo "准备安装最新的OS补丁------------"
echo "################################################################################" >> out.txt
echo "请自行判断系统是否是最新版本"  | tee -a out.txt
echo `uname -a`  | tee -a out.txt
echo "最新版本:RedHat Linux：http://www.redhat.com/support/errata/"  | tee -a out.txt
echo "最新版本:Slackware Linux：ftp://ftp.slackware.com/pub/slackware/"  | tee -a out.txt
echo "最新版本:SuSE Linux：http://www.suse.com/us/support/security/index.html"  | tee -a out.txt
echo "最新版本:TurboLinux：http://www.turbolinux.com/security/"  | tee -a out.txt
echo "#################################################################################" >> out.txt
